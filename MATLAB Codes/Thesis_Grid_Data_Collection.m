open_system('C:\Users\user1\Thesis_Grid_1deneme.slx');
tic
BlockPaths = find_system('Thesis_Grid_1deneme','Type','Block');
close all
...
rng(1234);
randomnewCs = normrnd(2.1E-11,3E-13,[1 50]);
randomnewGs = normrnd(3.5E-7,6E-8,[1 50]);
% Old Cable C Parameter per foot
C_perfoot_old = 2.1E-11*1.05;
G_perfoot_old = 3.5E-7*0.5;
randomoldCs = normrnd(2.1E-11*1.05,3E-13,[1 50]);
randomoldGs = normrnd(3.5E-7*0.5,6E-8,[1 50]);
...
%Calculate each Old cable parameters according to their length by using Long
%Transmission Line Model
for i =1 :length(Lengths)
   [GridRs_old(i), GridLs_old(i), GridRp_old(i), GridCp_old(i)]= Calculate_Cable_Parameters(f, Lengths(i), R_perfoot, L_perfoot, C_perfoot_old, G_perfoot_old);
end
